...
if isHDLcompatible
    SimParams.Rsym = 0.5e6;               % If HDL compatible, code will not be optimized in performance
    SimParams.CFCAlgorithm = 'Correlation-Based';
else
    if useCodegen
        SimParams.Rsym = 5e6;           % Symbol rate in Hertz in codegen path
        SimParams.CFCAlgorithm = 'FFT-Based';
    else
        SimParams.Rsym = 2.5e6;          % Symbol rate in Hertz
        SimParams.CFCAlgorithm = 'FFT-Based';
    end
end
